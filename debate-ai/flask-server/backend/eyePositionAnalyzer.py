...
# recording = cv.VideoCapture(r'C:\Users\subhr\Downloads\speech-recording.mp4')
...
def positionEstimator(cropped_eye):
    ...
    ret, threshed_eye = cv.threshold(median_blur, 100, 255, cv.THRESH_BINARY)
...
def pixelCounter(first_piece, second_piece, third_piece):
    ...
    if max_index == 0:
        ...
        pos_eye = "Closed"
        color = [utils.GRAY, utils.GREEN]
    return pos_eye, color

def analyzeEyePosition(video_recording_file_abs_path):
    frame_counter = 0
    center_frame_counter = 0
    CEF_COUNTER = 0
    TOTAL_BLINKS = 0
    # r'C:\Users\subhr\Downloads\speech-recording.mp4'
    recording = cv.VideoCapture(video_recording_file_abs_path)
    ...
    with map_face_mesh.FaceMesh(min_detection_confidence=0.5, min_tracking_confidence=0.5) as face_mesh:
        ...
        while True:
            ...
            ret, frame = recording.read()  # getting frame from video file
            if not ret:
            ...
            cv.imshow('Audio-Video Analyzer', frame)
            cv.setWindowProperty('Audio-Video Analyzer', cv.WND_PROP_TOPMOST, 1)
            key = cv.waitKey(2)
            if key == ord('q') or key == ord('Q'):
                break
        cv.destroyAllWindows()
        accuracy = (center_frame_counter/frame_counter)*100
        print("Accuracy: "+str(accuracy))
        print("Losing eye contact: " + str(100-accuracy))
        accuracy_ratio = {"maintainingEyeContactPercentage": accuracy, "losingEyeContactPercentage": 100-accuracy}
        return accuracy_ratio


if __name__ == "__main__":
    analyzeEyePosition(r'C:\Users\subhr\Downloads\speech-recording (2).mp4')